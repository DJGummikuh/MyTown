...
public abstract class MyTownDB extends Database {
	...
	public static DateFormat iso8601Format = new SimpleDateFormat(
			"yyyy-MM-dd HH:mm:ss");
	...
	private void update_21_11_2012() throws SQLException {
		try {
			PreparedStatement s = prepare("SELECT Extra FROM " + prefix
					+ "towns");
			s.executeQuery();
		} catch (SQLException e) {
			PreparedStatement statement = prepare("alter table " + prefix
					+ "towns ADD Extra varchar(2000) null");
	...
	private void update_13_12_2012() throws SQLException {
		try {
			PreparedStatement s = prepare("SELECT * FROM " + prefix
					+ "residents");
			s.executeQuery();
		} catch (SQLException e) {
			...
			PreparedStatement statementTown = prepare("SELECT * FROM " + prefix
					+ "towns");
				...
				if (res != null && res != "") {
					for (String split : res.split(" ")) {
						if (split.trim().length() > 0) {
							...
							PreparedStatement statement = prepare("DELETE FROM "
									+ prefix + "residents where Name = ?");
							statement.setString(1, rName);
							statement.executeUpdate();

							statement = prepare("INSERT INTO "
									+ prefix
									+ "residents (Name, Town, Rank, Channel, Created, LastLogin, Extra) VALUES (?, ?, ?, ?, ?, ?, ?)");
							statement.setString(1, rName);
							statement.setInt(2, tid);
							statement.setString(3, rRank.toString());
							statement.setString(4, rChannel.toString());
							statement.setString(5,
									iso8601Format.format(new Date(System
											.currentTimeMillis())));
	...
	private void update_14_12_2012() throws SQLException {
		try {
			PreparedStatement s = prepare("SELECT Friends FROM " + prefix
					+ "residents");
			s.executeQuery();
		} catch (SQLException e) {
			PreparedStatement statement = prepare("alter table " + prefix
					+ "residents ADD Friends TEXT");
			statement.executeUpdate();
		}
	}
	...
	private void update_27_02_2013() throws SQLException {
		PreparedStatement statement = prepare("alter table " + prefix
				+ "residents ADD Homes TEXT");
		statement.executeUpdate();
	}

	// ////////////////////////// end updates ////////////////////////////

	private void doUpdate(List<String> codes, String code, String desc)
			throws SQLException {
			...
			try {
				if (nation.id() > 0) {
					PreparedStatement statement = prepare("UPDATE "
							+ prefix
							+ "nations SET Name = ?, Towns = ?, Capital = ?, Extra = ? WHERE id = ?");
					...
					PreparedStatement statement = prepare(
							"INSERT INTO "
									+ prefix
									+ "nations (Name, Towns, Capital, Extra) VALUES (?, ?, ?, ?)",
							true);
					statement.setString(1, nation.name());
					statement.setString(2, sTowns);
					statement.setInt(3, nation.capital().id());
					...
					if (!rs.next())
						throw new RuntimeException(
								"Id wasn't returned for new nation "
										+ nation.name());
	...
	public void deleteTown(Town town) {
		synchronized (lock) {
			try {
				if (town.id() > 0) {
					PreparedStatement statement = prepare("DELETE FROM "
							+ prefix + "towns WHERE id = ?");
					statement.setInt(1, town.id());
					statement.executeUpdate();

					statement = prepare("UPDATE " + prefix
							+ "residents SET Town = 0, Rank = ? WHERE Town = ?");
	...
	public void saveResident(Resident res) {
		...
		synchronized (lock) {
			try {
				if (res.id() > 0) {
					updateResident(res, sFriends);
				} else {
					PreparedStatement checkForResStmt = prepare("SELECT id FROM "
							+ prefix
							+ "residents WHERE Name = ? ORDER BY id ASC");
					checkForResStmt.setString(1, res.name());
					ResultSet checkRS = checkForResStmt.executeQuery();
					if (checkRS.next()) {
						int firstID = checkRS.getInt("id");
						res.setId(firstID);
						updateResident(res, sFriends);
					} else {
						PreparedStatement statement = prepare(
								"INSERT INTO "
										+ prefix
										+ "residents (Name, Town, Rank, Channel, Created, LastLogin, Extra, Friends, Homes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
								true);
						statement.setString(1, res.name());
						statement.setInt(2, res.town() == null ? 0 : res.town()
								.id());
						statement.setString(3, res.rank().toString());
						statement.setString(4, res.activeChannel.toString());
						statement.setString(5,
								iso8601Format.format(res.created()));
						statement.setString(6,
						...
						if (!rs.next())
							throw new RuntimeException(
									"Id wasn't returned for new resident "
											+ res.name());
	...
	private void updateResident(Resident res, String sFriends)
			throws SQLException {
		PreparedStatement statement = prepare("UPDATE "
				+ prefix
				+ "residents SET Name = ?, Town = ?, Rank = ?, Channel = ?, LastLogin = ?, Extra = ?, Friends = ?, Homes = ? WHERE id = ?");
		statement.setString(1, res.name());
		...
		synchronized (lock) {
			try {
				if (town.id() > 0) {
					PreparedStatement statement = prepare("UPDATE "
							+ prefix
							+ "towns SET Name = ?, ExtraBlocks = ?, Blocks = ?, Extra = ? WHERE id = ?");
					...
					PreparedStatement statement = prepare(
							"INSERT INTO "
									+ prefix
									+ "towns (Name, ExtraBlocks, Blocks, Extra) VALUES (?, ?, ?, ?)",
							true);
					statement.setString(1, town.name());
					statement.setInt(2, town.extraBlocks());
					statement.setString(3, blocks.toString());
					...
					if (!rs.next())
						throw new RuntimeException(
								"Id wasn't returned for new town "
										+ town.name());
	...
	public List<Resident> loadResidents() {
		synchronized (lock) {
			...
			try {
				PreparedStatement statement = prepare("SELECT * FROM " + prefix
						+ "residents ORDER BY id");
				set = statement.executeQuery();
				List<String> residentNames = new LinkedList<String>();
				while (set.next()) {
					...
					if (residentNames.contains(r.name())) {
						// already loaded
						deleteResident(r);
						// we need to map the resident Id
					} else {
						residents.add(r);
					}
	...
	public Town loadFromSQL(int pId, String pName, int pExtraBlocks,
			String pBlocks, String pExtra) {
