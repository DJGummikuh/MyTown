...
public class Town {
	...
	public int residentBlockMultiplier(Resident res) {
		...
		if (ForgePerms.getPermissionsHandler().canAccess(res.name(),
                ...
                //res.onlinePlayer.worldObj.provider.getDimensionName(),"mytown.resident.blocksmulti.128"))
			return 128;
		if (ForgePerms.getPermissionsHandler().canAccess(res.name(),
                DimensionManager.getProvider(res.prevDimension).getDimensionName(),
                //res.onlinePlayer.worldObj.provider.getDimensionName(),"mytown.resident.blocksmulti.64"))
			return 64;
		if (ForgePerms.getPermissionsHandler().canAccess(res.name(),
                DimensionManager.getProvider(res.prevDimension).getDimensionName(),
                //res.onlinePlayer.worldObj.provider.getDimensionName(),"mytown.resident.blocksmulti.32"))
			return 32;
		if (ForgePerms.getPermissionsHandler().canAccess(res.name(),
                DimensionManager.getProvider(res.prevDimension).getDimensionName(),
                //res.onlinePlayer.worldObj.provider.getDimensionName(),"mytown.resident.blocksmulti.16"))
			return 16;

		if (ForgePerms.getPermissionsHandler().canAccess(res.name(),
                DimensionManager.getProvider(res.prevDimension).getDimensionName(),
                //res.onlinePlayer.worldObj.provider.getDimensionName(),"mytown.resident.blocksmulti.10"))
			return 10;
		if (ForgePerms.getPermissionsHandler().canAccess(res.name(),
                DimensionManager.getProvider(res.prevDimension).getDimensionName(),
                //res.onlinePlayer.worldObj.provider.getDimensionName(),"mytown.resident.blocksmulti.8"))
			return 8;
		if (ForgePerms.getPermissionsHandler().canAccess(res.name(),
                DimensionManager.getProvider(res.prevDimension).getDimensionName(),
                //res.onlinePlayer.worldObj.provider.getDimensionName(), "mytown.resident.blocksmulti.4"))
			return 4;
		if (ForgePerms.getPermissionsHandler().canAccess(res.name(),
                DimensionManager.getProvider(res.prevDimension).getDimensionName(),
                //res.onlinePlayer.worldObj.provider.getDimensionName(), "mytown.resident.blocksmulti.2"))
			return 2;
	...
	public int perResidentBlocks() {
		...
		// here we add bonus blocks if the number of residents exceeds specific
		// limits:
		// +20 when the town is 4 citizens or greater
		// +30 (cumultative) if the town is 7 citizens or greater
		if (residents.size() > 3)
			b += 20;
		if (residents.size() > 6)
			b += 30;
	...
	public static void canAddBlock(TownBlock block, boolean ignoreRoomCheck,
		...
		boolean omitRangeCheck = false;
		// Omit range check only when NOT in the main world
		// TODO: Make this configurable (which worlds to range-check)
		if (block.worldDimension() != 0) {
			omitRangeCheck = true;
		}
		if (block.town() != null)
